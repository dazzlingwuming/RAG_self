'''
对于HyDE（Hypothetical Document Embeddings）方法的实现，核心思想是通过大模型生成与用户查询相关的假设性问题，从而进行多查询检索，以克服基于距离的相似性搜索的一些局限性。
之前是通过q去查询相似的文档，但是这是问题对答案，但是问题不一定包括答案的所有信息，所以通过先使用大模型回答问题，生成相关的答案查询，再去检索，从而提高检索的全面性和准确性。
'''
import os
...
def rephrase_retrieve(input: Dict[str, str], llm, retriever):
    ...
    hyde_prompt = PromptTemplate.from_template(
        """
        你是一名AI语言模型助理。你的任务是生成给定问题的相关答案，即使不了解问题的确切答案，也要根据你的知识生成一个合理的答案。原始问题：{query}
        """
    )
    ...
    multi_query_chain = (rephrase_chain
                         | (lambda x: {"query": x})  # 添加一个参数，用于控制生成多少个查询
                         | expend_query_chain  # 生成多个查询
                         | (lambda x: print(f"===== 假设性答案: {x}=====") or x)
                         | (lambda x: [retriever.invoke(x, k=3)])  # 遍历检索多个查询
                         # | (lambda x: print(x) or x)  # 打印检索结果
                         )
...
if __name__ == "__main__":
    ...
    # 重述+多查询+检索
    retrieve_result = rephrase_retrieve(input_data, llm=llm, retriever = retriever)
