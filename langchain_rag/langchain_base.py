...
from untils.untils_rag import format_docs_queries


#调用大模型api
def get_llm():
    # 大模型
    load_dotenv(config.dotenv_path)  # 加载环境变量文件 .env
...
def llm_chain_unretrieve(llm):
    ...
    prompt = PromptTemplate.from_template(
        '''你是一个专业的中文问答助手，擅长基于提供的资料回答问题。
请仅根据历史消息和你本身的知识回答问题，如无法找到答案，请直接回答“我不知道”。
